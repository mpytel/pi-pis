...
const bootPis = async (): Promise<Pis> => {
  ...
  const piPiType: Pi = new PiBase("pi", "piType", "pi sub-class type designator.");
  rtnPis.pis.push(piPiType);
  const piPiTypePiType: Pi = new PiBase("piType", "piType", "The pyType used to designate a pi as a piType.");
  rtnPis.pis.push(piPiTypePiType);
  const piTypePi: Pi = new PiBase("piType", "pi", "The pyType used to designate a pi as a pi piType.");
  rtnPis.pis.push(piTypePi);
  const piTypePiField: Pi = new PiBase("piType", "piField", "A pi that defines and discribes a pi opbject key.");
  ...
  const piFieldName: Pi = new PiBase("piField", "name", "The application name used when a pi was created.");
  rtnPis.pis.push(piFieldName);
  const piFieldVersion: Pi = new PiBase("piField", "version", "The application version number when a pi was created.");
  rtnPis.pis.push(piFieldVersion);
  const piFieldPiType: Pi = new PiBase("piField", "piType", "The name of the pi's type");
  rtnPis.pis.push(piFieldPiType);
  const piFieldPiTitle: Pi = new PiBase("piField", "piTitle", "The name of the pi.");
  rtnPis.pis.push(piFieldPiTitle);
  const piFieldPiSD: Pi = new PiBase("piField", "piSD", "A short discription of what the pi is.");
  ...
  const piIDPiUser: Pi = new PiBase("piID", "piUser", "System level pi storing a the root user name.");
  ...
  const piIDPiDomain: Pi = new PiBase("piID", "piDomain", "System level pi storing subject domaine and its decendets.");
